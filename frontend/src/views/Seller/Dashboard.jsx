    ...
    useEffect(() => {
        ...
        const loadDashboardData = async () => {
            try {
                ...
                setRecentOrders([
                    { id: 'ORD-001', customer: 'João Silva', date: '2026-01-07', total: 4500, status: 'pending' },
                    { id: 'ORD-002', customer: 'Maria Santos', date: '2026-01-06', total: 2100, status: 'shipped' },
                    { id: 'ORD-003', customer: 'Pedro M.', date: '2026-01-06', total: 15000, status: 'delivered' },
                ]);
            } catch (error) {
                console.error(error);
            } finally {
        ...
        <Card className="border-0 shadow-sm h-100">
            <Card.Body className="d-flex align-items-center justify-content-between">
                <div>
                    ...
                    {trend && (
                        <small className={`d-flex align-items-center mt-2 ${trend > 0 ? 'text-success' : 'text-danger'}`}>
                            {trend > 0 ? <ArrowUpRight className="me-1" /> : <ArrowDownRight className="me-1" />}
        ...
        <Container fluid className="px-0">
            ...
            <Card className="border-0 shadow-sm">
                <Card.Header className="bg-white border-0 py-3 d-flex justify-content-between align-items-center">
                    ...
                    <Link to="/seller/orders" className="btn btn-sm btn-outline-primary rounded-pill">
                        Ver Todos
                    </Link>
                </Card.Header>
                        ...
                        <Table hover className="mb-0 align-middle">
                            <thead className="bg-light">
                                <tr>
                                    ...
                                    <th className="border-0 text-end pe-4">Ação</th>
                                </tr>
                            </thead>
                            <tbody>
                                {recentOrders.map(order => (
                                    <tr key={order.id}>
                                        ...
                                        <td>{order.date}</td>
                                        <td>{order.total.toLocaleString()} MT</td>
