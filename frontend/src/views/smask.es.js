const numberFormat = (value, locale = navigator.language, options = {}) => {
  options = typeof options == 'string' ? { style: options } : {style: 'decimal', ...options};
...
const currencyFormat = (value, locale = navigator.language, currency) => {
    
    if (!currency) {
        for (let language of navigator.languages) {
            if (language.indexOf('-') != -1)  currency = currencyLocale[language.slice(-2).toUpperCase()]
        }
    }
    console.log(currency)
  return numberFormat(value, locale, {style: "currency",  currency });
};
...
//# sourceMappingURL=smask.es.js.map
