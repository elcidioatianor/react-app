...
xhr.transformRequest(async (config) => {//REQUEST LOGGER
...
xhr.transformResponse(async (res) => {//add next()
	try {
...
xhr.interceptError(async (error, config) => {
    ...
    if (error.status === 401 && error.code === errors.EACCESS_EXPIRED) {
        ...
        try {
            ...
            //const xhrRefresh = new XHR(baseURL);
            //TODO: USAR GET
            ...
            if (accessToken) {
                ...
                console.log('Refresh Ok! Reenviando request...')
                // Executar requisições enfileiradas
                //Retentar requisicao original
				//refreshCount = 0;
            ...
            //refreshCount = 0;
            //navigate('/login')
            //TODO: ANALIZE EREFRESH E REFRESH_EXPIRY
            throw refreshError;
        }
